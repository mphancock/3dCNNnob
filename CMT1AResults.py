...
def splitData(matPath, dataPath):
    testFileID = [20010, 20020, 20030, 20090, 20150, 20180, 20200, 20210, 1150560, 1150570, 1162010, 1279360, 1279830,
                  2146490]

    badFileID = [20160, 20310, 1300650, 2131220, 2161250]
    dupFileID = [20022, 1150560, 1162010, 1279830]

    testFileID = [str(file) for file in testFileID]
    badFileID = [str(file) for file in badFileID]
    dupFileID = [str(file) for file in dupFileID]

    trainPath = os.path.join(dataPath, 'train')
    testPath = os.path.join(dataPath, 'test')

    fileList = os.listdir(matPath)
    fileList = [file for file in fileList if '.mat' in file]
    ...
    for file in fileList:
        ...
        if name in testFileID:
            process_file(file=file,
                         data_path=testPath,
                         mat_path=matPath)
            # print(file, ' saved to test data set')
        elif name not in badFileID and name not in dupFileID:
            process_file(file=file,
                         data_path=trainPath,
                         mat_path=matPath)
            # print(file, ' saved to train data set')
        else:
            print(file, ' omitted from data set')
